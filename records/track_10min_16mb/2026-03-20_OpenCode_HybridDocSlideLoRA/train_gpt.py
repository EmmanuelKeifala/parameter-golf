...
class Hyperparameters:
    ...
    run_id = os.environ.get("RUN_ID", "opencode_hybrid_docslide_lora")
    ...
    eval_doc_isolated = bool(int(os.environ.get("EVAL_DOC_ISOLATED", "1")))
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_delta_fn=None, v_delta_fn=None) -> Tensor:
        ...
        n = self.attn_norm(x)
        q_delta = q_delta_fn(n) if q_delta_fn is not None else None
        v_delta = v_delta_fn(n) if v_delta_fn is not None else None
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None, reduction: str = "mean") -> Tensor:
        ...
        if reduction == "none":
            return F.cross_entropy(logits_flat, targets, reduction="none").reshape_as(target_ids)
        if reduction != "mean":
            raise ValueError(f"Unsupported reduction: {reduction}")
        return F.cross_entropy(logits_flat, targets, reduction="mean")
...
def _build_ttt_optimizer(lora: nn.Module, args: Hyperparameters) -> torch.optim.Optimizer:
    return torch.optim.Adam(lora.parameters(), lr=args.ttt_lora_lr, betas=(args.beta1, args.beta2), eps=1e-10)

def find_document_boundaries(all_tokens: Tensor, bos_id: int) -> list[tuple[int, int]]:
...
def build_doc_sliding_windows(docs: list[tuple[int, int]], seq_len: int, stride: int) -> list[tuple[int, int, int, int]]:
    windows: list[tuple[int, int, int, int]] = []
    for doc_start, doc_len in docs:
        pred_len = doc_len - 1
        for score_start in range(0, pred_len, stride):
            score_end = min(score_start + stride, pred_len)
            ...
            windows.append((doc_start + window_start, window_len, score_offset, score_len))
    return windows
...
def eval_val_doc_sliding(
    ...
    windows = build_doc_sliding_windows(find_document_boundaries(val_tokens, bos_id), seq_len, stride)
    ...
    with torch.inference_mode():
        for i in range(0, len(my_windows), eval_batch_seqs):
            ...
            bs = len(batch)
            x = torch.zeros(eval_batch_seqs, seq_len, dtype=torch.int64, device=device)
            y = torch.zeros(eval_batch_seqs, seq_len, dtype=torch.int64, device=device)
            for b, (abs_start, window_len, _, _) in enumerate(batch):
                chunk = val_tokens[abs_start : abs_start + window_len + 1].to(device=device, dtype=torch.int64)
                x[b, :window_len] = chunk[:-1]
                y[b, :window_len] = chunk[1:]

            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = logits_fn(x)

            for b, (_, _, score_offset, score_len) in enumerate(batch):
                scored_logits = logits[b, score_offset : score_offset + score_len]
                scored_targets = y[b, score_offset : score_offset + score_len]
                loss_sum += F.cross_entropy(scored_logits.float(), scored_targets, reduction="sum").to(torch.float64)
                tok_count += score_len
                prev = x[b, score_offset : score_offset + score_len]
                tb = base_bytes_lut[scored_targets].to(torch.int16)
                tb += (has_leading_space_lut[scored_targets] & ~is_boundary_token_lut[prev]).to(torch.int16)
...
def eval_val_doc_sliding_ttt_lora(
    args: Hyperparameters,
    base_model: GPT,
    rank: int,
    world_size: int,
    ...
    docs = find_document_boundaries(val_tokens, bos_id)
    rank_docs = docs[(len(docs) * rank) // world_size : (len(docs) * (rank + 1)) // world_size]
    rank_docs.sort(key=lambda item: item[1])
    ...
    shared_opt = _build_ttt_optimizer(shared_lora, args)
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    tok_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    for i in range(0, len(rank_docs), batch_size):
        batch = rank_docs[i : i + batch_size]
        bsz = len(batch)
        if bsz == batch_size:
            lora = shared_lora
            opt = shared_opt
        ...
        max_chunks = max((pl + chunk_size - 1) // chunk_size for pl in pred_lens)
        for ci in range(max_chunks):
            ...
            for b, (doc_start, doc_len) in enumerate(batch):
                pred_len = doc_len - 1
                score_start = ci * chunk_size
                if score_start >= pred_len:
                    meta.append((0, 0, False))
                    continue
                score_end = min(score_start + chunk_size, pred_len)
                window_end = score_end
                window_start = max(0, window_end - seq_len)
                window_len = window_end - window_start
                score_offset = score_start - window_start
                score_len = score_end - score_start
                chunk = val_tokens[doc_start + window_start : doc_start + window_start + window_len + 1].to(device=device, dtype=torch.int64)
                x[b, :window_len] = chunk[:-1]
                y[b, :window_len] = chunk[1:]
                meta.append((score_offset, score_len, score_end < pred_len))

            active = any(score_len > 0 for _, score_len, _ in meta)
            if not active:
                continue
            ...
            with torch.no_grad():
                for b, (score_offset, score_len, _) in enumerate(meta):
                    if score_len <= 0:
                        continue
                    lbl = ptl[b, score_offset : score_offset + score_len].to(torch.float64)
                    tgt = y[b, score_offset : score_offset + score_len]
                    prev = x[b, score_offset : score_offset + score_len]
            ...
            if any(should_train for _, score_len, should_train in meta if score_len > 0):
                train_losses = []
                for b, (score_offset, score_len, should_train) in enumerate(meta):
                    if score_len <= 0:
                        continue
                    train_losses.append(ptl[b, score_offset : score_offset + score_len].mean() if should_train else ptl[b, score_offset : score_offset + score_len].mean() * 0.0)
                opt.zero_grad()
    ...
    val_loss = float(loss_sum.item() / tok_count.item())
    return val_loss, float((loss_sum.item() / math.log(2.0)) / byte_count.item())
...
def main() -> None:
    ...
    log0(f"tokenizer_bos_id:{bos_id}")
    ...
    needs_full_val_tokens = args.eval_doc_isolated or args.ttt_enable
    val_tokens_full = load_all_validation_tokens(args.val_files) if needs_full_val_tokens else val_tokens
    if eval_sl != args.train_seq_len and not args.eval_doc_isolated:
        ...
        val_tokens_eval = val_tokens_full if args.eval_doc_isolated else val_tokens
    ...
    if eval_stride > 0:
        ...
        if args.eval_doc_isolated:
            q_val_loss, q_val_bpb = eval_val_doc_sliding(
                compiled_logits,
                rank,
                world_size,
                device,
    ...
    if args.ttt_enable and args.ttt_lora_rank > 0:
        ttt_sl = args.ttt_eval_seq_len if args.ttt_eval_seq_len > 0 else eval_sl
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        ttt_val_loss, ttt_val_bpb = eval_val_doc_sliding_ttt_lora(
            args,
            base_model,
            rank,
            world_size,
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int8_docslide_ttt_lora val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
        )
        log0(f"final_int8_docslide_ttt_lora_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}")
